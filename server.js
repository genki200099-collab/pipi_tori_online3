...
function cpuUnwantedValue(room, player, card){
  ...
  const mode = normalizePenaltyMode(room.penaltyMode);
  const mad = room.madPigEnabled !== false && card.suit==='♠' && card.rank==='11';
  if(mad) return mode === 'faceValue' ? 900000 : 700000;

  // 残り手札として重くなりやすいカードを優先して候補に入れる。
  // 数字分失点では高数字ほど不要。1枚-3点でも、高数字は勝ち札として強い反面、終盤に残ると心理的負担が大きいのでやや優先。
  let value = Number(card.val || 0) * (mode === 'faceValue' ? 100 : 35);

  // 同じ数字のペアがあるカードは後で浄化できる可能性があるため、少しだけ候補優先度を下げる。
  const sameRank = (player?.hand || []).filter(c=>!c.joker && c.rank===card.rank).length;
  if(sameRank >= 2) value -= 60;

  // 低いカードはトリックで逃げやすいため、不要度は低め。
  if(Number(card.val || 0) <= 3) value -= 50;
  return value;
}
...
function roomOptionSummary(room){
  return `全${room.totalRounds || 3}R / 失点:${roomPenaltyLabel(room)} / ババ:-${room.jokerPenalty ?? 20} / マッド:${roomMadPigLabel(room)} / ピック:${pickTargetLabel(room)} / 3枚パス:${room.passThreeEnabled ? 'あり' : 'なし'} / 開始ペア:${room.initialPairDiscardEnabled ? 'あり' : 'なし'}`;
}

function createRoom(ws, name, totalRounds=3, madPigEnabled=true, jokerPenalty=-20, initialPairDiscardEnabled=false, passThreeEnabled=false, penaltyMode='flat3', pickTargetCount=0){
  const c = code();
  const id = uid();
